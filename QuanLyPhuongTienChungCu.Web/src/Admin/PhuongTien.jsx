...
function PhuongTien() {
  ...
  const [tuKhoaCanHo, setTuKhoaCanHo] = useState("");

  const [loading, setLoading] = useState(true);
  const [loadingCanHo, setLoadingCanHo] = useState(false);
  const [loadingLoaiXe, setLoadingLoaiXe] = useState(false);
  const [dangLuu, setDangLuu] = useState(false);

  const [loi, setLoi] = useState("");
  const [loiCanHo, setLoiCanHo] = useState("");

  const [hienThiModal, setHienThiModal] = useState(false);
  const [hienThiDropdownCanHo, setHienThiDropdownCanHo] =
    useState(false);
  ...
  const taiDanhSach = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/PhuongTien`, {
        method: "GET",
        headers: taoHeaders(),
      });
  ...
  const taiDanhSachCanHo = async () => {
    try {
      setLoadingCanHo(true);
      ...
      setDanhSachCanHo(Array.isArray(data) ? data : []);
    } catch (error) {
  ...
  useEffect(() => {
    const xuLyClickNgoai = (event) => {
      if (
        ...
        !apartmentRef.current.contains(event.target)
      ) {
        setHienThiDropdownCanHo(false);
      }
    };
  ...
  const danhSachCanHoLoc = useMemo(() => {
    const keyword = tuKhoaCanHo
      .trim()
      .toLowerCase();
    ...
    return danhSachCanHo.filter((item) => {
      return (
        String(item.maCanHo || "")
          .toLowerCase()
          .includes(keyword) ||
  ...
  }, [danhSachCanHo, tuKhoaCanHo]);

  // =====================================================
  // CĂN HỘ ĐANG CHỌN
  // =====================================================

  const canHoDangChon = useMemo(() => {
    const value = form.maCanHo
      .trim()
      .toUpperCase();
  ...
  const chonCanHo = (canHo) => {
    setForm((prev) => ({
      ...prev,
      maCanHo: canHo.maCanHo,
    }));

    setTuKhoaCanHo(canHo.maCanHo);
  ...
  const thayDoiCanHo = (event) => {
    ...
    setTuKhoaCanHo(value);

    setHienThiDropdownCanHo(true);
    ...
    const tonTai = danhSachCanHo.some(
      (item) =>
        String(item.maCanHo)
          .trim()
          .toUpperCase() ===
  ...
  const kiemTraForm = () => {
    ...
    if (!form.bienSo.trim()) {
      setLoi("Vui lòng nhập biển số xe.");
      return false;
    }
    ...
    const loaiXeTonTai =
      danhSachLoaiXe.some(
        (item) =>
          ...
          Number(form.loaiPhuongTienId)
      );

    if (
      danhSachLoaiXe.length > 0 &&
      !loaiXeTonTai
    ) {
      setLoi(
    ...
    const canHo = danhSachCanHo.find(
      (item) =>
        String(item.maCanHo)
          .trim()
          .toUpperCase() ===
        ...
        <div className="pt-table-box">
          ...
          <div className="pt-table-toolbar">
            ...
            <div className="pt-total">
              {danhSachLoc.length} phương tiện
            </div>
          </div>
                        ...
                        <td>
                          <span
                            className={
                              ...
                              "ACTIVE"
                                ? "pt-status pt-status-active"
                                : "pt-status pt-status-inactive"
                            }
                          >
