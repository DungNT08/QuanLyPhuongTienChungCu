...
const QLyUser = () => {
  ...
  const moThem = () => {
    setDangSua(null);
    setForm({ tenDangNhap: "", hoTen: "", vaiTro: "Cư dân", trangThai: "Hoạt động" });
  ...
  const luu = () => {
    ...
    if (dangSua) {
      ...
      const idMoi = danhSach.length ? Math.max(...danhSach.map((u) => u.id)) + 1 : 1;
    ...
    <div className="nd-wrapper">
      <div className="nd-body">
        <div className="nd-title-bar">
          <h2>Người dùng</h2>
          <button className="nd-btn-add" onClick={moThem}>+ Thêm người dùng</button>
        </div>
              ...
              {danhSachLoc.length === 0 ? (
                ...
                danhSachLoc.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.hoTen}</td>
                    <td>{item.vaiTro}</td>
                    <td><span className="nd-status">{item.trangThai}</span></td>
                    <td className="nd-actions">
                      <button className="nd-edit" onClick={() => moSua(item)}>✏️</button>
                      <button className="nd-delete" onClick={() => xoa(item.id)}>🗑️</button>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ background: "#fff", padding: 24, borderRadius: 10, minWidth: 400, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Tên đăng nhập *</label>
              <input type="text" value={form.tenDangNhap} onChange={(e) => setForm({ ...form, tenDangNhap: e.target.value })}
                style={{ width: "100%", padding: 8, border: "1px solid #dce7ed", borderRadius: 6 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Họ tên *</label>
              <input type="text" value={form.hoTen} onChange={(e) => setForm({ ...form, hoTen: e.target.value })}
                style={{ width: "100%", padding: 8, border: "1px solid #dce7ed", borderRadius: 6 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Vai trò</label>
