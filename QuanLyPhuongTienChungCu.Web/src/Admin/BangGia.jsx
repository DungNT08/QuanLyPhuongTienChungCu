...
const BangGia = () => {
  ...
  const luu = () => {
    ...
    if (dangSua) {
      setDanhSach((prev) => prev.map((b) => (b.id === dangSua ? { ...b, ...form } : b)));
    } else {
      const idMoi = danhSach.length ? Math.max(...danhSach.map((b) => b.id)) + 1 : 1;
    ...
    <div className="bg-wrapper">
      <div className="bg-body">
        <div className="bg-title-bar">
          ...
          <button className="bg-btn-add" onClick={moThem}>+ Thêm bảng giá</button>
        </div>
        ...
        <div className="bg-table-box">
                ...
                danhSach.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td className="bg-actions">
                      <button className="bg-edit" onClick={() => moSua(item)}>✏️</button>
                      <button className="bg-delete" onClick={() => xoa(item.id)}>🗑️</button>
                    </td>
                  </tr>
                ))
              )}
          ...
          <div className="bg-pagination">
            <span>Hiển thị {danhSach.length > 0 ? 1 : 0} - {danhSach.length} trong {danhSach.length} bảng giá</span>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ background: "#fff", padding: 24, borderRadius: 10, minWidth: 400, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Loại phương tiện *</label>
              <input type="text" value={form.loaiPhuongTien} onChange={(e) => setForm({ ...form, loaiPhuongTien: e.target.value })}
                style={{ width: "100%", padding: 8, border: "1px solid #dce7ed", borderRadius: 6 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Đơn giá (giờ)</label>
              <input type="number" value={form.donGiaGio} onChange={(e) => setForm({ ...form, donGiaGio: Number(e.target.value) })}
                style={{ width: "100%", padding: 8, border: "1px solid #dce7ed", borderRadius: 6 }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Đơn giá (ngày)</label>
              <input type="number" value={form.donGiaNgay} onChange={(e) => setForm({ ...form, donGiaNgay: Number(e.target.value) })}
                style={{ width: "100%", padding: 8, border: "1px solid #dce7ed", borderRadius: 6 }} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", marginBottom: 4, fontSize: 13, fontWeight: 600 }}>Ghi chú</label>
